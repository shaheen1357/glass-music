...
final class PlayerEngine: ObservableObject {
    ...
    @Published private(set) var gaplessEnabled = true         // remove the silence between tracks
    @Published private(set) var crossfadeDuration: Double = 0 // seconds (0...12); 0 = off
    @Published private(set) var autoplayEnabled = true        // keep playing similar songs when the queue ends
    ...
    /// the last position the display timer published (clock.currentTime).
    private func capturedPlayhead() -> AVAudioFramePosition {
        let byClock = currentFrame
        let byTime = AVAudioFramePosition(max(0, clock.currentTime) * sampleRate)
    ...
    private func maybeBeginTransition() {
        ...
        let remaining = duration - clock.currentTime
    ...
    private func commitTransition(incomingFile: AVAudioFile, next: Track) {
        ...
        clock.currentTime = sampleRate > 0 ? Double(currentFrame) / sampleRate : 0
    ...
    private func savePlaybackState() {
        ...
        let state = PersistedState(
            ...
            currentTime: max(0, clock.currentTime),
    ...
    func persistNow() {
        if sampleRate > 0 { clock.currentTime = Double(currentFrame) / sampleRate }
        savePlaybackState()
    }
